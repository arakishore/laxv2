...
if (!noError($conn)) {
   ...
   $reportMonthYear = "";
    if (isset($_GET["reportMonthYear"])) {
        $selectedDate = $reportMonthYear =  cleanQueryParameter($conn, cleanXSS($_GET["reportMonthYear"]));
    }
   $nd  = "";
   if (isset($_GET["nd"])) {
  ...
  //print_r($checkUnAssignedContentOwner);
  $type = "youtube_whp_report";
?>
<!doctype html>
<html lang="en">
    ...
    <div class="header" id="youtube1">
        ...
        <div class="row">
            ...
            <div class="col-lg-3">
                ...
                <h4 class="modal2-title">Validate Report Youtube Claim v2 <?php echo $nd?></h4>
...
//(int)$checkUnAssignedContentOwner > 0 &&  $checkUnAssignedContentOwner["errCode"] == "-1"
        ...
        <div class="row">
            <div class="col-md-12 ">
                <fieldset class="alert alert-danger">
                    ...
                    <div>
                        <div>
                            <span>Systems show #<?php echo $checkUnAssignedContentOwner?> content owner is Not assigned
                                <a class="btn btn-success" id="btnExportunAssigned" data-toggle="collapse" href="#"
                                    style="margin-left:0.5vw;">Export</a></span>
                            <div id="alert" class="alert alert-default" style="display: none;">
                            </div>
                            File path : /var/lib/mysql-files/nonassignedcontentowners/<?php echo $table_name?>.csv
                        </div>
                    </div>
                </fieldset>
            </div>
            <!-- <div class="col-md-6" id="alert21">
                <div id="alert2" class="alert alert-danger" style="display: none ;">
                    <span></span>
                </div>
                <fieldset id="div2" class="alert alert-danger">
                    <div>
                        
                        <form method="post" name="frmassigncontentowner" id="frmassigncontentowner"
                            action="update_content_owner_v2.php" enctype="multipart">
                            <div class="">
                                <input type="hidden" name="nd" id="nd" value="<?php echo $_GET["nd"]?>">
                                <input type="hidden" name="reportMonthYear" id="reportMonthYear"
                                    value="<?php echo $selectedDate?>">
                                <input type="hidden" name="table_name" id="table_name" value="<?php echo $table_name?>">
                                <input type="hidden1" name="type" id="type" value="<?php echo $type?>">
                                
                                Upload File (<?php echo $table_name?>.csv)
                                <input type="hidden" name="filename" id="filename" value="<?php echo $table_name?>.csv">
                                <div class="row">
                                    <div class="col-md-6">
                                        <input type="file" class="form-control" required="" id="assigned_content_owner"
                                            name="assigned_content_owner" value="">
                                    </div>
                                    <div class="col-md-6">
                                        <button type="submit" class="chkND" name="btnUpload" id='btnUpload'
                                            onclick1="saveReport('assigned_content_owner');">
                            ...
                            <div>
                            * Update will be on the basis of (Youtube: Channel ID)</br>
                                * Upload same file as exported from left window/side </br>
                                * Don't change any column name, Sequence of column </br>
                            </div>
                        </form>
    ...
    <table id="example" class="table table-striped table-hover" style="width:100%">
        <thead>
            <tr>
                ...
                <th>AdSense Earnings Month</th>
                <th>Channel ID</th>
                <th>Revenue Source</th>
                <th>Local Currency</th>
                <th>Total Channel Revenue</th>
                <th>US Sourced Revenue</th>
                <th>Tax Withholding Rate</th>
                <th>Tax Withheld Amount</th>
                <th>Content Owner</th>
...
function saveReport(textvalue) {
    ...
    //  alert(revenueShareYoutubeUpload+" " + filename );

    if (revenueShareYoutubeUpload != filename) {

        $("#alert2").
        removeClass("alert-success").
        addClass("alert-danger").
        fadeIn().
        find("span").html("File name should be  " + filename);
        return;
    }
    //
    // frmassigncontentowner.submit();
}
...
$(document).ready(function() {
    $("form#frmassigncontentowner").submit(function(e) {
        ...
        //Assigning in Process. Please reload the page after some time
        ...
        if (1) {
            ...
            fadeIn().
            find("span").html("Assigning in Process. Please reload the page after some time");
           // return;
        }
        ...
        $.ajax({
            url: "update_content_owner_v2.php",
            type: 'POST',
            data: formData,
            success: function(data) {
              ...
              if (1) {
                    ...
                    addClass("alert-success").
                    fadeIn().
                    find("span").html("Assigning of content owner is completed. Please click <a href='<?php echo basename($_SERVER["PHP_SELF"]); ?>?reportMonthYear=<?php echo $reportMonthYear?>&nd=<?php echo $nd?>'>here to repload the page</a>");
                // return;
                }
            },
            cache: false,
...
$(document).ready(function() {
    ...
    var table = $('#example').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "../../controller/validate/youTube_withholding_percentage_report_v2.php",
            "data": function(d) {
                return $.extend({}, d, {
                    "search_keywords": $("#searchInput").val().toLowerCase(),
                    "filter_option": $("#sortBy").val().toLowerCase(),
                    "report": 'youtube_whp_report',
                    "reportMonthYear": '<?php echo $_GET["reportMonthYear"]?>',
                    "nd": '<?php echo $_GET["nd"]?>'
                });
            }
