           ...
           if (empty($userInfo)) {
               ...
               if ($userInfo[$email]["status"]!=1) {
                  ...
                  $clientSearchArr["content_owner"] = NULL;
                   ...
                   $allClientsCount = getActivationContentOwnerecordsGroup1(
                    $table_type_name,
                    $conn,
                   ...
                   $allClientsCount = sizeof( $allClientsCount["errMsg"]); //why anonymous? see function definition
    ...
    <div id="alert" class="alert alert-default" style="display: none;">
        ...
        <div class="card">
            ...
            <div class="card-content">
                    ...
                    <tbody>
                        ...
                        foreach($allClientsInfo as $clientEmail=>$clientDetails){ 
                         ...
                         //   $us_payout = $clientDetails["us_payout"];
                            ...
                            $content_owner_expl = explode(",",$clientDetails["content_owner"]);
                            //print_r($content_owner_expl);

                            $content_owner_expl = make_array_unique($content_owner_expl);
                            //print_r($content_owner_expl);
                            
                            $clientDetails["content_owner"]  = implode(", ",$content_owner_expl);
                           // print_r($clientDetails["content_owner"]);
                            
                            //clientDetails["`content_owner`"]
                          ...
                          //  $amount_cmsType = getAmountRecived($conn,$table_type_name,$clientDetails["content_owner"]);
                            ...
                            $slab_percentage = get_sharespercentage_groupcontentowner($conn,$total_amt_recd,$witholding,$clientDetails["group_name"],$shares);
                            $differencep =0;
                            if($shares!=""){
                                $shares_exp = explode(",",$shares);
                                foreach($shares_exp as $skey => $sahreValue){
                                    $sahreValue = trim($sahreValue);
                                    if($slab_percentage!=$sahreValue){
                                        $differencep =1;
                                    }
                                }
                            }
                       ...
                       //  $differencep = $slab_percentage - $shares;
                        ...
                        <td><?php echo $clientDetails["group_name"]; ?></td>
                            ...
                            <td><?php echo str_replace(",","</br>",$clientDetails["tablename"]); ?></td>
                ...
                <!-- pagination -->
                <nav aria-label="Page navigation" style="display:none1">
                    <ul class="pagination">
                        <?php
...
$(document).ready(function() {
    ...
    function saveContentowner() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "<?php echo $rootUrl; ?>controller/activate/generate/changePercentageGroup.php",
            data: {
