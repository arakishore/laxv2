...
if (!noError($conn)) {
    ...
    //$client_id = $clientInfo['errMsg'][$email]['client_id'];
     ...
     if (!noError($clientInfo)) {
        ...
        $activatetableName  = 'youtube_video_claim_activation_report_nd%_'.$year.'_'.$month;
    ...
    $finance_report_table = 'youtube_video_claim_report_nd1_'.$year.'_'.$month;
    $youtube_report_table = 'youtube_video_claim_report_nd1_'.$year.'_'.$month;
    ...
    $_SESSION['client'] =  $myclientname;
     ...
     $status_flag_withholding_actual = 0;
...
////
    $sql = "select * from activity_downlaod_report where  type_table='{$type_table}' and content_owner = '{$_SESSION['client']}' and selected_date='{$selected_date}' and type_cate='{$type_cate}' and downlaodType='status_flag_withholding_actual' limit 0,1 ";
    $resultQyery = runQuery($sql, $conn);
    $resultQyeryscheck = mysqli_num_rows($resultQyery["dbResource"]);
   
    if ($resultQyeryscheck > 0) {
        $resultQyerydata = mysqli_fetch_assoc($resultQyery["dbResource"]);
        $status_flag_withholding_actual =$resultQyerydata['status_flag'];
...
<body>
    ...
    <div class="header" id="youtube1">
        ...
        <div class="row">
            ...
            <div class="col-lg-3">
                ...
                <h4 class="modal2-title">Youtube Claim <?php echo htmlspecialchars($_GET['reportMonthYear']); ?></h4>
    ...
    <div class="col-md-4">
        ...
        $fileis = $_SESSION['client'] . '_YoutubeClaim_report_' . $year . '_' . $month . '.zip';
    ...
    <div class="col-md-4">
        ...
        $fileis = $_SESSION['client'] . '_YoutubeClaim_report_' . $year . '_' . $month . 'withholding.zip';
        if (file_exists('../../../excelreports/' . $fileis)) {
            ?>
        <a href='../../../excelreports/<?=$fileis?>'><button type="button" name="btnDownload" id="btnDownload"
  ...
  $table_type_name = 'youtube_video_claim_activation_report_nd%_' . $year . '_' . $month;
  ...
  ?>
       ...
       // $final_payable_us = (isset($activationdata_us_report['final_payable'])) ? $activationdata_us_report['final_payable'] : 0   ;
        $final_payable_us_with_gst = (isset($activationdata_us_report['final_payable_with_gst'])) ? $activationdata_us_report['final_payable_with_gst'] : 0   ;
            ...
            <?php
            if(SHOW_WHM_MODULE==1 || $_GET['SHOW_WHM_MODULE']==1){
                $table_type_name = "youtube_video_claim_report_nd%" . $year . '_' . $month;
                $other_tables  =  "youtube_video_claim_report2_nd%". $year . '_' . $month;
                $table_type_name_whm = 'youtube_whp_report_nd%_' . $year . '_' . $month;

                    $uspayout = (!empty($activationdata['us_payout'])) ? $activationdata['us_payout'] : 0;
                    $uspayment_holding_amount = (!empty($activationdata['witholding'])) ? $activationdata['witholding'] : 0;
            ...
            if (!noError($allClientsInfo_whm_report)) {
                ...
                if(!empty($allClientsInfo_whm_report)){
                   ...
                   $new_amt = $allClientsInfo_whm_report_data['new_amt'];
                   $old_amt = $allClientsInfo_whm_report_data['old_amt'];
       
                 //  print_r($allClientsInfo_whm_report);
                   if($new_amt < $uspayment_holding_amount) {
                       $balance_amt = $uspayment_holding_amount - $new_amt;
                       $message = "{$balance_amt} will addd in in next payment";
                   } else {
                       $balance_amt =  $new_amt - $uspayment_holding_amount;
                       $message = "{$balance_amt} will deducted  in in next payment";
                   }
                }
        ...
        if($status_flag_withholding_actual==0 || $status_flag_withholding_actual==3 ){
            ?>
        <!-- <button type="submit" name="export_withholding" id="export_withholding" class="btn btn-warning fa fa-download" style="font-size:20px;">With Holding Actual
        </button> -->
        <?php }  else {

        $fileis = $_SESSION['client'] . '_YoutubeClaim_report_' . $year . '_' . $month . 'withholding_actual.zip';
        if (file_exists('../../../excelreports/' . $fileis)) {
            ?>
        <a href='../../../excelreports/<?=$fileis?>'><button type="button" name="btnDownload" id="btnDownload"
                class="btn btn-info">Download zip (With Actual Holding)</button></a>
        <?php
        } else {
            ?>
        <div id="alertaaa" class="text-center alert alert-info">Report generation is in queue, will notify via mail
