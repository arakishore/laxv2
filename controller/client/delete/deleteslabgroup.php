...
if (isset($_POST["group_name"])) {
    $group_name = cleanQueryParameter($conn, cleanXSS(urldecode($_POST["group_name"])));
}
...
if (empty($group_name)) {
    ...
    $returnArr["errCode"] = 4;
    $returnArr["errMsg"] = getErrMsg(4)." group_name cannot be empty";
    echo(json_encode($returnArr));
    exit;
}
...
$deleteClient = deleteClientGroup($clientSearchArr, $conn);
if (!noError($deleteClient)) {
    ...
    $returnArr["errCode"] = 5;
    $returnArr["errMsg"] = "Error deleting group_name. Please try again after some time.";
    echo(json_encode($returnArr));
    exit;
}

//Everything completed successfully
$logMsg = "Client group name deleted successfully. Return response";
...
$returnArr["errCode"] = -1;
$returnArr["errMsg"] = getErrMsg(-1)." Client group name deletion successful.";
echo(json_encode($returnArr));
exit;
?>