...
$importFailureEmailMessage = "<div>
    <p>There was an error in Export youtube whp   data on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error Export youtube whp   data into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $logMsg = "Validating arguments: ".json_encode($_GET);
    $logData["step3"]["data"] = "3. {$logMsg}";

    $emailSubject = "Generate Activate Youtube Report whp";

    //validate filepath
    
     $youtube_report_table = (isset($_GET['table'])) ? $_GET['table'] : '';
    $client = (isset($_GET['client'])) ? $_GET['client'] : '';

 
    $logMsg = "All parameters are valid. Attempting to start generating whp";
    ...
    $allClientsInfocount  = ExportWhpreportv10($youtube_report_table, $conn,$client);
    
    if (!noError($allClientsInfocount)) {

 
        $returnArr["errCode"] = 5;
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details. whp";
    } else {  
         ...
         if ($export) {
            $searchdata='';
            $logMsg = "Request is to export all clients data to excel.whp";
            $logData["step6"]["data"] = "6. {$logMsg}";
     
            $xcelfiles = [];
            $headtitle = ['AdSense Earnings Month','Channel ID','Revenue Source','Local Currency', 'Total Revenue By Source', 'US Sourced Revenue','Tax Withholding Rate', 'Tax Withheld Amount'];
            ...
            for($i=0;$i<$noofexcels;$i++){
                ...
                $allClientsInfo  = ExportWhpreportv10($youtube_report_table ,$conn,$client,$offset,$resultsPerPage);
        ...
        $returnArr["errCode"] = -1;
        header('Content-disposition: attachment; filename="'.$zip_file_tmp.'"');
